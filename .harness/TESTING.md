...
| `tests/unit/test_v3_structured_agent.py` | 26 | Structured LLM retries, evidence-only validation, ArchitectureAgent mock behavior, V3 single-agent report contract, real LLM opt-in guard, no_findings_reason parsing, Chinese prompt rules, bilingual display fields, code fence stripping, validator severity preservation, and timing attribution parsing. |
