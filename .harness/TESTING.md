...
`pytest --collect-only -q` collected 261 tests.

| Layer | Tests | Files | Purpose |
|-------|-------|-------|---------|
| Unit | 241 | 28 | Validate contexts, prompts, structured reviews, review state, backend services, parsers, sandbox safety, evidence, structured LLM agents, multi-agent orchestration, V3 hardening, lifecycle, API errors, LLM behavior, report generation, storage, and task runner. |
