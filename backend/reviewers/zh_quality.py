...
_PHRASE_REPLACEMENTS: dict[str, str] = {
    # Repository/report metadata fragments
...
def _split_inline_code(text: str) -> list[tuple[str, bool]]:
    """Split text into (segment, is_inline_code) pieces."""
    segments: list[tuple[str, bool]] = []
    pattern = re.compile(r"`[^`]*`")
    last_end = 0
...
def _replace_standalone_term(text: str, source: str, target: str) -> str:
    """Replace a standalone English term without touching identifiers or paths."""
    if "/" in source:
...
def _normalize_mixed_count_phrases(text: str) -> str:
    """Normalize English count phrases that often appear in zh summaries."""
    result = text
    result = re.sub(
        r"\b(\d+)\s+Python source files\b",
        r"\1 个 Python 源文件",
        result,
        flags=re.IGNORECASE,
    )
    result = re.sub(
        r"\banalyzed\s+(\d+)\s+and\s+skipped\s+(\d+)\b",
        r"已分析 \1 个，已跳过 \2 个",
        result,
        flags=re.IGNORECASE,
    )
    result = re.sub(
        r"\b(\d+)\s+of\s+(\d+)\s+supported source files\b",
        r"已分析 \1 / \2 个支持的源文件",
        result,
        flags=re.IGNORECASE,
    )

    def _severity_count(match: re.Match[str]) -> str:
        count = match.group(1)
        severity = match.group(2).lower()
        return f"{count} 个{_COUNT_SEVERITY_REPLACEMENTS[severity]}"

    result = re.sub(
        r"\b(\d+)\s+(critical|high|medium|low|info|informational)\b",
        _severity_count,
        result,
        flags=re.IGNORECASE,
    )
    return re.sub(
        r",\s+(?=\d+ 个(?:严重风险|高风险|中风险|低风险|信息项))",
        "，",
        result,
    )


def _normalize_zh_plain_text(text: str) -> str:
    """Normalize non-code Chinese display text."""
    result = _normalize_mixed_count_phrases(text)

    for en, zh in sorted(_PHRASE_REPLACEMENTS.items(), key=lambda x: -len(x[0])):
        result = _replace_standalone_term(result, en, zh)

    for en_label, zh_label in _INLINE_LABEL_REPLACEMENTS.items():
        result = re.sub(
            rf"(?<![a-zA-Z]){re.escape(en_label)}(?::(?= )|(?![a-zA-Z-]))",
            zh_label,
            result,
        )
    ...
    for en_sev, zh_sev in _SEVERITY_REPLACEMENTS.items():
        ...
        result = result.replace(f"| {en_sev} |", f"| {zh_sev} |")
        result = result.replace(f"| {en_sev}|", f"| {zh_sev}|")
        if result.strip().lower() == en_sev:
            result = zh_sev
        else:
            result = _replace_standalone_term(result, en_sev, zh_sev)
    ...
    for en_st, zh_st in _STATUS_REPLACEMENTS.items():
        ...
        result = result.replace(f"| {en_st} |", f"| {zh_st} |")
        result = result.replace(f"| {en_st}|", f"| {zh_st}|")
        if result.strip().lower() == en_st:
            result = zh_st
        else:
            result = _replace_standalone_term(result, en_st, zh_st)

    for en_value, zh_value in _TEXT_VALUE_REPLACEMENTS.items():
        if result.strip().lower() == en_value:
            result = zh_value
        else:
            result = _replace_standalone_term(result, en_value, zh_value)

    return _RAW_EV_RE.sub("[E?]", result)


def _normalize_zh_non_code_text(text: str) -> str:
    """Normalize text while preserving inline code spans."""
    return "".join(
        segment if is_code else _normalize_zh_plain_text(segment)
...
def normalize_zh_text(text: str | None) -> str | None:
    ...
    if text is None:
        return None
    return _normalize_zh_non_code_text(text)
...
def normalize_zh_markdown(markdown: str) -> str:
    ...
    for segment_text, is_code in segments:
        ...
        result_parts.append(_normalize_zh_non_code_text(segment_text))
